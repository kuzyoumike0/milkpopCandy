(() => {
  "use strict";
  console.log("[app.js] LOADED v16.2 (WB merge safe + per-bunny charge + unchi separated)", Date.now());
  ...
  const ASSETS = {
    ...
    // unchi / ougonUnchi は unchi.js 側へ分離する（ここには置かない）
    coins: [
  ...
  const LS = {
    ...
    dex:       "wb_dex_v1",
    unchi:     "wb_unchi_v1",       // ✅ unchi.js が使う（app.js は触らない）
    title:     "wb_title_v1",
  ...
  // index.html 側の実態に合わせて optional
  const shopBtn     = document.getElementById("shopBtn");      // hidden互換用
  const omukaeBtn   = document.getElementById("omukaeBtn");    // ✅ 今のHUDのメイン
  const hanabiBtn   = document.getElementById("hanabiBtn");    // ✅ 花火（hanabi.js側が使う）
  const departBtn   = document.getElementById("departBtn");    // ある構成もある
  ...
  function getSeVolume() {
    try {
      if (typeof window.WB?.getSEVolume === "function") return clamp(Number(window.WB.getSEVolume()) || 0.85, 0, 1);
    } catch {}
  ...
  /* =========================
   * CSS injection（うさぎ見えない事故を根絶 + コイン小さく + ハート）
   * ========================= */
  (function injectCssOnce() {
    if (document.getElementById("wbAppCoreCssV162")) return;
    const st = document.createElement("style");
    st.id = "wbAppCoreCssV162";
    st.textContent = `
      /* bunny表示の最低保証 */
  ...
  function spawnClickCoins(bunny, count = 1, tierPicker = () => 0) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      spawnCoinDropAt(x, y, tierPicker());
    }
  }
  ...
  class Bunny {
    constructor(bornAt, kind = "bunny1") {
      ...
      // 画像が404でも「座標更新が止まらない」よう、念のため即適用
      this.clampInside();
      this.applyPos();
    }
  ...
  // hanabiBtn は hanabi.js が直接使う想定（ここでは触らない）
  ...
  if (resetBtn) {
    resetBtn.addEventListener("click", () => {
      ...
      localStorage.removeItem(LS.unchi);     // ✅ unchi.js の記録も消える
  ...
  const api = {
    ...
    // コイン生成（外部からも使える。unchi.js が「うんち叩いたらコイン散る」等に使える）
    spawnCoinDropAt,
