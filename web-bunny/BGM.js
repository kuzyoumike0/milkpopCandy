(() => {
  "use strict";
  console.log("[app.js] LOADED v17.0 (WB merge + fix bgm modal z + unchi above bunny)", Date.now());
  ...
  // ★排出率：ここを上げるほど頻繁（例：0.18 → 0.198 = +10%）
  ...
  function ensureUnchiLayer() {
    ...
    layer.style.zIndex = "120";         // ✅ うさぎより上・モーダルより下
    field.appendChild(layer);
    return layer;
  }
  const unchiLayer = ensureUnchiLayer();
  ...
  function unlockAudioOnce() {
    ...
    // 既存WB側のunlockも呼ぶ（BGM.jsが持ってる可能性）
  ...
  /* =========================
   * CSS injection
   * - ハートz-indexを下げてモーダルを邪魔しない
   * - BGMモーダルが裏に潜る保険CSSも追加
   * ========================= */
  (function injectCssOnce() {
    ...
    st.textContent = `
      ...
      .unchiDrop{
        ...
        pointer-events:auto;
        cursor:pointer;
        z-index: 122; /* ✅ unchiLayer(120)の中で上 */
      }

      .wbChargeHart {
        position:absolute;
        z-index:130; /* ✅ 9999→下げる：モーダルより下 */
      ...
      /* ✅ 保険：BGMモーダルが何かに負けても最前面へ */
      #bgmModal, #bgmBackdrop, #bgmPanel,
      [id*="bgm"][id*="Modal"], [id*="bgm"][id*="Backdrop"], [id*="bgm"][id*="Panel"]{
