...
//        + ✅購入したBGMが流れない問題の対策（audio.load + play失敗理由toast + 追い再生 + 再試行））
...
(() => {
  ...
  function saveBgmSettings(s) { try { localStorage.setItem(LS_KEY_BGM_SETTINGS, JSON.stringify(s)); } catch {} }
  ...
  function saveSeVol(v) { try { localStorage.setItem(LS_KEY_SE_VOL, String(clamp(Number(v) || 0, 0, 1))); } catch {} }
...
`;
  ...
  function safeToast(msg) {
    try { toast(msg); return; } catch {}
    try { if (typeof window.WB?.toast === "function") window.WB.toast(msg); return; } catch {}
  ...
  async function tryPlayBgm(src, keyHint = null) {
    ...
    const onError = () => {
      try {
        ...
        safeToast(`⚠️ BGM読み込み失敗：${__lastPlayErr}`);
      } catch (e) {
        console.warn(e);
      }
    };
  ...
  function stopBgm() { if (!audio) return; try { audio.pause(); } catch {} }
  ...
  async function startPreview(key) {
    ...
    try {
      ...
      if (p && typeof p.then === "function") await p;
      safeToast(`👂 試聴中：${LABELS[key] || key}`);
      return true;
    } catch (e) {
      safeToast(`⚠️ 試聴失敗：${String(e?.name || e?.message || e || "preview play failed")}`);
      return false;
    }
  }
  ...
  function stopPreview({ resumeBgm = true } = {}) {
    ...
    if (resumeBgm && bgmSettings.enabled && unlocked) startBgm(true);
  }
  ...
  function getSEVolume() { return clamp(Number(window.__milkpopSeVolume ?? seVolume), 0, 1); }
  ...
  function setSEVolume(v) {
    ...
    try { for (const a of seRegistry) if (a && typeof a.volume === "number") a.volume = seVolume; } catch {}
    try { for (const a of loopMap.values()) if (a && typeof a.__base === "number") a.volume = clamp(a.__base * seVolume, 0, 1); } catch {}
  }
  ...
  function sePlay(src, base = 1.0) {
    try {
      if (!unlocked) return;
      const a = new Audio(toAbsUrlEncoded(src));
      a.preload = "auto";
      a.loop = false;
  ...
  function playSpecial(keyOrSrc) {
    ...
    if (TRACKS[keyOrSrc]) {
      if (!isOwned(keyOrSrc)) { safeToast("未購入です"); return; }
      specialKey = keyOrSrc;
  ...
  function clearSpecial() { specialKey = null; startBgm(true); }
  ...
  function removeOldUI() {
    ...
    for (const id of ids) { try { document.getElementById(id)?.remove(); } catch {} }
  }
...
#bgmShopV3 .preview.active{ background:#2b6cff; color:#fff; box-shadow:none; }
`;
...
`;
    ...
    for (const k of keys) {
      ...
      buyBtns[k]?.addEventListener("click", async () => {
        ...
        const r = buyBgm(k);
        if (!r.ok) {
          if (r.reason === "coins") safeToast(`🪙 足りない！ ${r.have} / ${r.need}`);
          else safeToast("購入できませんでした");
        } else {
          stopPreview({ resumeBgm: false });
          selectBgm(k);
        }
  ...
  function drainRegisterQueue() {
    try {
      ...
      while (q.length) registerSE(q.shift());
    } catch {}
  }
  ...
  function startTimeWatcher() { setInterval(() => startBgm(false), 30_000); }
