// BGM.js（非module / ✅購入＆選択UIが「必ず出る」版 + ✅SE音量調整 + ✅追加BGM購入）V2.1
...
// ✅ 追加：SE音量スライダー（WB.getSEVolume / WB.setSEVolume）
// ✅ 追加：添付mp3を購入可能に（Cocktail / Stream / おもしろすぎてどっかん）
...
(() => {
  ...
  // ✅ SE volume
  function loadSeVol() {
    const v = Number(localStorage.getItem(LS_SE_VOL) ?? "0.85");
    return clamp(Number.isFinite(v) ? v : 0.85, 0, 1);
    }
  function saveSeVol(v) {
    const x = clamp(Number(v), 0, 1);
    try { localStorage.setItem(LS_SE_VOL, String(x)); } catch {}
    // app.js が fallback で参照する場合もある
    try { window.__milkpopSeVolume = x; } catch {}
    return x;
  }
...
`;
  ...
  async function tryPlay(src, keyHint = null) {
    ...
    // ✅ 日本語ファイル名対策：encodeURI
    const safeSrc = encodeURI(src);

    const nextHref = new URL(safeSrc, location.href).href;
  ...
  function patchWB(WB) {
    ...
    if (lastWBRef === WB && WB.__bgmPatchedV21) return;
    lastWBRef = WB;

    if (!WB.__bgmPatchedV21) WB.__bgmPatchedV21 = { done: false };
    ...
    // ✅ SE volume API（app.js が getSEVolume を参照）
    WB.getSEVolume = () => seVolume;
    WB.setSEVolume = (v) => {
      seVolume = saveSeVol(v);
      return seVolume;
    };

    WB.__bgmPatchedV21.done = true;
...
<div style="margin-top:8px;">
  <div class="fine" style="font-weight:900; opacity:.85;">🎵 BGM音量</div>
  <input class="slider" id="bgmVolV2" type="range" min="0" max="100" step="1" />
</div>

<div style="margin-top:6px;">
  <div class="fine" style="font-weight:900; opacity:.85;">🔊 SE音量</div>
  <input class="slider" id="seVolV2" type="range" min="0" max="100" step="1" />
  <div class="fine" style="opacity:.7;">※クリックSE/コインSE/ぽよSEなど</div>
</div>

<div class="fine" id="bgmInfoV2" style="margin-top:8px;"></div>
...
<div id="bgmShopV2">
  ...
  ${renderItem("depart", LABELS.depart, "特別BGM（旅立ち演出など）")}
  <div class="sep"></div>
  <div class="fine" style="font-weight:900; opacity:.85;">🎶 追加BGM</div>
  ${renderItem("cocktail", LABELS.cocktail, "添付mp3")}
  ${renderItem("stream", LABELS.stream, "添付mp3")}
  ${renderItem("dokkan", LABELS.dokkan, "添付mp3")}
</div>
`;
    ...
    function refresh() {
      ...
      seVol.value = String(Math.round(seVolume * 100));
    ...
    seVol.addEventListener("input", () => {
      seVolume = saveSeVol(Number(seVol.value) / 100);
      // app.js 側が WB.getSEVolume を参照していれば即反映される
      try { window.WB?.setSEVolume?.(seVolume); } catch {}
      refresh();
    });
  ...
  // ✅ gameMenu.js から呼べる「モーダル的open」
  function openModal() {
    if (!document.getElementById(UI.hamburger) || !document.getElementById(UI.panel)) {
  ...
  (async function boot() {
    ensureAudio();
    // app.js が fallback で読む場合に備えて初期値をセット
    try { window.__milkpopSeVolume = seVolume; } catch {}
